...
export const decodeMetaData = async (metadata: any) => {
  let decoder = new TextDecoder('utf-8');
  let arrayBuffer = new Uint8Array(metadata).buffer;
  let text = decoder.decode(arrayBuffer);
  let metaJson = JSON.parse(text);
  return metaJson;
}

export const wordToByteArray = (word: any, length: any): number[] => {
...
export const lebDecode = (pipe: any) => {
  let weight = BigInt(1);
  let value = BigInt(0);
  let byte;
  do {
